...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    vector<Store*> pointersToAuxStores;
    for (int i = 0; i < auxStores.size(); i++) {
        Store* p = &auxStores[i];
        pointersToAuxStores.push_back(p);
    }

    // TODO:  Sort the vector of pointers using the STL sort algorithm
    //        with the comp parameter as the ordering relationship.
    for (int unsorted = 1; unsorted < pointersToAuxStores.size(); unsorted++) {
        Store* nextItem = pointersToAuxStores[unsorted];
        int loc = unsorted;
        while (loc > 0 && comp(nextItem, pointersToAuxStores[loc - 1])) {
            pointersToAuxStores[loc] = pointersToAuxStores[loc - 1];
            loc--;
        }
        pointersToAuxStores[loc] = nextItem;
    }
    ...
    for (int i = 0; i < stores.size(); i++) {
        stores[i] = *pointersToAuxStores[i];
    }
